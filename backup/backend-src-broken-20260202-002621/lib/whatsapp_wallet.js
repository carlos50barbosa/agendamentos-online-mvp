...
export const WHATSAPP_TOPUP_PACKAGES = [ { messages: 100, priceCents: 990 }, { messages: 200, priceCents: 1690 }, { messages: 300, priceCents: 2490 }, { messages: 500, priceCents: 3990 }, { messages: 1000, priceCents: 7990 }, { messages: 2500, priceCents: 19990 }, ];
...
op: opName, estabelecimentoId: estId, attempt, delayMs, errno: err?.errno, code: err?.code, });
}
...
return {
id: pack.id != null ? toInt(pack.id, null) : null, code: pack.code || null, name: pack.name || null, messages, priceCents, };
}

function findTopup(messages, availablePacks = null) {
const m = toInt(messages, 0);
const candidates = Array.isArray(availablePacks) && availablePacks.length ? availablePacks : WHATSAPP_TOPUP_PACKAGES;
...
await conn.query(
    `INSERT IGNORE INTO whatsapp_wallets
     ...
     VALUES (?, ?, ?, ?, ?, 0)`, [estabelecimentoId, cycle.start, cycle.end, includedLimit, includedLimit] );
}
...
const [[row]] = await conn.query(
    `SELECT estabelecimento_id, cycle_start, cycle_end, included_limit, included_balance, extra_balance
     ...
     LIMIT 1${lockClause}`, [estabelecimentoId] );
if (!row) return null;
...
const needsCycleReset = !currentStart || !currentEnd || Number.isNaN(currentStart.getTime()) || Number.isNaN(currentEnd.getTime()) || currentStart.getTime() !== cycle.start.getTime() || currentEnd.getTime() !== cycle.end.getTime();
if (needsCycleReset) {
await conn.query(
      `UPDATE whatsapp_wallets
       SET cycle_start=?, cycle_end=?, included_limit=?, included_balance=?, updated_at=CURRENT_TIMESTAMP
       WHERE estabelecimento_id=?
       LIMIT 1`, [cycle.start, cycle.end, includedLimit, includedLimit, estabelecimentoId] );
await conn.query(
      `INSERT IGNORE INTO whatsapp_wallet_transactions
       ...
       VALUES (?, 'cycle_reset', ?, ?, 0, ?, ?, ?)`, [
        estabelecimentoId, includedLimit, includedLimit, cycle.start, cycle.end, safeJson({ included_limit: includedLimit, cycle: { start: cycle.start, end: cycle.end } }), ]
    );
return {
estabelecimento_id: row.estabelecimento_id, cycle_start: cycle.start, cycle_end: cycle.end, included_limit: includedLimit, included_balance: includedLimit, extra_balance: row.extra_balance, };
}
...
await conn.query(
      `UPDATE whatsapp_wallets
       ...
       WHERE estabelecimento_id=?
       LIMIT 1`, [includedLimit, nextBalance, estabelecimentoId] );
return {
estabelecimento_id: row.estabelecimento_id, cycle_start: row.cycle_start, cycle_end: row.cycle_end, included_limit: includedLimit, included_balance: nextBalance, extra_balance: row.extra_balance, };
}

  return {
estabelecimento_id: row.estabelecimento_id, cycle_start: row.cycle_start, cycle_end: row.cycle_end, included_limit: currentLimit, included_balance: currentBalance, extra_balance: toInt(row.extra_balance, 0), };
}
...
const wallet = await withWalletTransaction( estId, async (conn) => {
...
return {
estabelecimento_id: estId, month_label: cycle.label, cycle_start: wallet.cycle_start ? new Date(wallet.cycle_start).toISOString() : null, cycle_end: wallet.cycle_end ? new Date(wallet.cycle_end).toISOString() : null, included_limit: includedLimitOut, included_balance: includedBalance, extra_balance: extraBalance, total_balance: includedBalance + extraBalance, plan: ctx?.plan || 'starter', plan_status: ctx?.status || null, };
}

export async function recordWhatsAppBlocked({
estabelecimentoId, agendamentoId = null, reason, metadata, }) {
const estId = toInt(estabelecimentoId, 0);
...
await pool.query(
    `INSERT INTO whatsapp_wallet_transactions
     ...
     VALUES (?, 'blocked', 0, 0, 0, ?, ?, ?)`, [estId, agendamentoId != null ? toInt(agendamentoId, 0) || null : null, String(reason || ''), safeJson(metadata)] );
return { ok: true };
}

export async function creditWhatsAppTopup({
estabelecimentoId, messages, paymentId, subscriptionId = null, metadata, pack = null, }) {
const estId = toInt(estabelecimentoId, 0);
...
const result = await withWalletTransaction( estId, async (conn) => {
...
const [ins] = await conn.query(
        `INSERT IGNORE INTO whatsapp_wallet_transactions
         ...
         VALUES (?, 'topup_credit', ?, 0, ?, ?, ?, ?, ?)`, [
          estId, pkg.messages, pkg.messages, subscriptionId != null ? toInt(subscriptionId, 0) || null : null, String(paymentId), reason, safeJson({
...(metadata || {}), pack_code: pkg.code || metadata?.pack_code || null, pack_id: pkg.id || metadata?.pack_id ?? null, pack_name: (pkg.name || metadata?.pack_name) ?? null, messages: pkg.messages, price_cents: pkg.priceCents, }), ]
      );
if (!ins.affectedRows) {
...
estabelecimentoId, agendamentoId = null, providerMessageId, metadata, }) {
const estId = toInt(estabelecimentoId, 0);
if (!estId) return { ok: false, error: 'missing_estabelecimento' };
...
const [ins] = await conn.query(
        `INSERT IGNORE INTO whatsapp_wallet_transactions
         ...
         VALUES (?, 'debit', -1, ?, ?, ?, ?, ?)`, [
          estId, includedDelta, extraDelta, agendamentoId != null ? toInt(agendamentoId, 0) || null : null, String(providerMessageId), safeJson(metadata), ]
      );
if (!ins.affectedRows) {
return { idempotent: true };
}
...
try { parsed = JSON.parse(row.metadata); } catch { parsed = null; }
    ...
    return {
id: row.id, delta: toInt(row.delta, 0), included_delta: toInt(row.included_delta, 0), extra_delta: toInt(row.extra_delta, 0), payment_id: row.payment_id ? String(row.payment_id) : null, metadata: parsed, created_at: row.created_at ? new Date(row.created_at).toISOString() : null, };
});
}
