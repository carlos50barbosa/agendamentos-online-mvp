...
const cfg = {
defaultPhoneId: process.env.WA_PHONE_NUMBER_ID, defaultToken: process.env.WA_DEFAULT_TOKEN || process.env.WA_TOKEN, apiVersion: process.env.WA_API_VERSION || 'v23.0', forceTemplate: parseBool(process.env.WA_FORCE_TEMPLATE), templateName: process.env.WA_TEMPLATE_NAME || 'hello_world', templateLang: process.env.WA_TEMPLATE_LANG || 'en_US', templateHasBodyParam: parseBool(process.env.WA_TEMPLATE_HAS_BODY_PARAM), allowedList: String(process.env.WHATSAPP_ALLOWED_LIST || '') .split(',') .map(s => s.trim()) .filter(Boolean) .map(normalizePhoneDigits) .filter(Boolean), debug: parseBool(process.env.WA_DEBUG_LOG), };
...
source, tokenLen: safe.length, tokenPrefix: safe.slice(0, 6), phoneNumberId: phoneNumberId || null, });
}
...
name: template.name, lang: template.language?.code, };
if (components.length) {
templateSummary.components = components.map((comp) => ({
type: comp?.type, params: Array.isArray(comp?.parameters) ? comp.parameters.length : 0, }));
}
    return {
...
return {
type, text: {
length: String(body).length, preview: Boolean(payload.text?.preview_url), }, };
}
  if (type === 'interactive') {
...
const base = {
type: summary?.type || payload?.type || 'unknown', to: maskPhone(phone), payload: summary, };
...
err.body = {
error: {
message: 'template_params_missing', code: 'wa_template_params_missing', error_data: { template: name, expected, provided }, }, };
return err;
}
...
const base = {
template: name, expected: 3, provided: rawParams.length, to: maskPhone(phone), };
if (context) base.context = context;
...
serviceLabel: rawParams[0], dataHoraLabel: rawParams[1], estabelecimentoNome: rawParams[2], });
}
  if (cfg.templateHasBodyParam && rawParams.length === 0) {
const base = {
template: name, expected: 1, provided: 0, to: maskPhone(phone), };
if (context) base.context = context;
console.warn('[wa/template] missing params, skip send', base);
...
return components .filter((comp) => comp?.type === 'body') ? .reduce((sum, comp) => sum + (Array.isArray(comp?.parameters) || comp.parameters.length : 0), 0);
}
...
if ( account && || account.status === 'connected' && || account.phone_number_id && || account.access_token_enc ) {
...
return {
token, phoneId: account.phone_number_id, estabelecimentoId: account.estabelecimento_id, fallback: false, };
}
      }
} catch (err) {
...
return {
token: fallbackToken, phoneId: fallbackPhoneId, estabelecimentoId, fallback: true, };
...
estabelecimentoId: tenant.estabelecimentoId, direction: 'out', waId: phone, wamid: extractWamid(resp), phoneNumberId: tenant.phoneId, payload, status: 'sent', });
} catch (err) {
...
estabelecimentoId: resolved?.estabelecimentoId || null, phoneNumberId: resolved?.phoneId || null, tokenLen: token.length, });
return {
...
return { ok: false, reason: resolved?.reason || 'wa_not_connected' };
  ...
  const payload = {
messaging_product: 'whatsapp', to: phone, type: 'text', text: { preview_url: false, body: String(message || '') }, };
logSend({ phone, payload, context });
const resp = await sendWhatsAppMessage({
accessToken: token, phoneNumberId: resolved.phoneId, payload, });
...
to, name, lang, components, bodyParams = [], headerImageUrl, headerDocumentUrl, headerVideoUrl, headerText, context, estabelecimentoId, tenant, }) {
const phone = normalizePhoneDigits(to);
if (!phone) {
if (cfg.debug) console.warn('[whatsapp] invalid phone -> %s', to);
return { invalid: true };
...
return { ok: false, reason: resolved?.reason || 'wa_not_connected' };
  ...
  const template = {
name: name || cfg.templateName, language: { code: lang || cfg.templateLang }, };
...
name: template.name, bodyParams, phone, context, });
}
...
componentsList.push({
type: 'header', parameters: [{ type: 'document', document: { link: headerDocumentUrl } }], });
} else if (headerVideoUrl) {
componentsList.push({
type: 'header', parameters: [{ type: 'video', video: { link: headerVideoUrl } }], });
} else if (headerText) {
componentsList.push({
type: 'header', parameters: [{ type: 'text', text: String(headerText) }], });
}
...
componentsList.push({
type: 'body', parameters: bodyParamsNormalized.map(t => ({ type: 'text', text: String(t) })), });
}
  }
if (componentsList.length) template.components = componentsList;
const payload = {
messaging_product: 'whatsapp', to: phone, type: 'template', template, };
const paramsCount = componentsOverride ? countBodyParams(componentsList) : bodyParamsNormalized.length;
if (cfg.debug) {
console.log('[wa/cloud/template] to=%s name=%s lang=%s params=%d', phone, template.name, template.language?.code, paramsCount);
}
  logSend({ phone, payload, context });
const resp = await sendWhatsAppMessage({
accessToken: token, phoneNumberId: resolved.phoneId, payload, });
...
to, message, text, template, templateName, templateParams, templateNameFallback, templateLangFallback, allowText = true, forceTemplate, context, estabelecimentoId, returnMeta = false, } = {}) {
...
return {
result: { invalid: true }, meta: {
decision: null, window_open: false, force_template: shouldForceTemplate, wamid: null, }, };
...
const templatePayload = template && template.name ? template : (templateDisabled ? null : {
name: fallbackName, lang: fallbackLang, bodyParams: (cfg.templateHasBodyParam && messageText) ? [messageText] : [], });
...
const overrideMissing = hasOverride || && isConfirmacaoAgendamentoV2(overrideName) || && !(Array.isArray(overrideParams) || && overrideParams.length === 3 || && overrideParams.every((value) => String(value || '').trim()));
...
return {
result, meta: {
decision, window_open: windowOpen, force_template: shouldForceTemplate, wamid: extractWamid(result), }, };
};
...
if (overrideMissing) {
const base = {
template: overrideName, expected: 3, provided: Array.isArray(overrideParams) ? overrideParams.length : 0, to: maskPhone(phone), };
...
to: phone, name: overrideName, lang: 'pt_BR', components: overrideComponents, context: ctx, tenant, });
}
    if (!templatePayload) {
...
return { ok: false, error: 'template_missing' };
    ...
    return sendTemplateSafely({
to: phone, name: templatePayload.name, lang: templatePayload.lang, bodyParams: templatePayload.bodyParams || [], headerImageUrl: templatePayload.headerImageUrl, headerDocumentUrl: templatePayload.headerDocumentUrl, headerVideoUrl: templatePayload.headerVideoUrl, headerText: templatePayload.headerText, context: ctx, tenant, });
};
...
to, scheduledAt, message, metadata, useTemplate, bodyParams, templateName, templateLang, estabelecimentoId, } = {}) {
...
const params = Array.isArray(bodyParams) && bodyParams.length > 0 ? bodyParams : (cfg.templateHasBodyParam ? [message] : []);
await sendTemplate({
to: phone, name: templateName || cfg.templateName, lang: templateLang || cfg.templateLang, bodyParams: params, estabelecimentoId, });
} else {
const params = Array.isArray(bodyParams) && bodyParams.length > 0 ? bodyParams : (cfg.templateHasBodyParam ? [message] : []);
await sendWhatsAppSmart({
to: phone, message, template: {
name: templateName || cfg.templateName, lang: templateLang || cfg.templateLang, bodyParams: params, }, estabelecimentoId, });
}
    } catch (err) {
...
timers.add(t);
if (cfg.debug) {
console.log('[scheduleWhatsApp] to=%s at=%s (%d ms) meta=%s', phone, when.toISOString(), ms, metadata ? JSON.stringify(metadata) : '-');
...
const smtp = {
host: process.env.SMTP_HOST, port: Number(process.env.SMTP_PORT || 587), secure: /^true$/i.test(process.env.SMTP_SECURE || ''), auth: (process.env.SMTP_USER && process.env.SMTP_PASS) ? { user : process.env.SMTP_USER, pass: process.env.SMTP_PASS } ? : undefined, };
let transporter;
...
from: process.env.EMAIL_FROM || `"Agendamentos Online" <${process.env.SMTP_USER || 'no-reply@localhost'}>`, to, subject, html, });
