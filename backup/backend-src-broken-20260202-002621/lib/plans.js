...
const PLAN_CONFIG = {
starter: {
code: 'starter', label: 'Starter', priceCents: 1490, annualPriceCents: 14900, maxMonthlyAppointments: LIMIT_UNLIMITED, maxServices: LIMIT_UNLIMITED, maxProfessionals: 2, maxGalleryImages: 5, allowWhatsApp: true, allowAdvancedReports: false, whatsappIncludedMessages: 250, whatsappMaxMessagesPerAppointment: 5, }, pro: {
code: 'pro', label: 'Pro', priceCents: 2990, annualPriceCents: 29900, maxMonthlyAppointments: LIMIT_UNLIMITED, maxServices: LIMIT_UNLIMITED, maxProfessionals: 5, maxGalleryImages: 15, allowWhatsApp: true, allowAdvancedReports: true, whatsappIncludedMessages: 500, whatsappMaxMessagesPerAppointment: 5, }, premium: {
code: 'premium', label: 'Premium', priceCents: 9990, annualPriceCents: 99900, maxMonthlyAppointments: LIMIT_UNLIMITED, maxServices: LIMIT_UNLIMITED, maxProfessionals: 10, maxGalleryImages: LIMIT_UNLIMITED, allowWhatsApp: true, allowAdvancedReports: true, whatsappIncludedMessages: 1500, whatsappMaxMessagesPerAppointment: 5, }, };
export const BILLING_CYCLES = {
mensal: {
key: 'mensal', label: 'Mensal', frequency: 1, frequencyType: 'months', }, anual: {
key: 'anual', label: 'Anual', frequency: 1, frequencyType: 'years', }, };
...
const daysLeft = rawDaysLeft != null && Number.isFinite(rawDaysLeft) ? Math.max(0, Math.min(rawDaysLeft, MAX_TRIAL_DAYS)) : null;
...
const [[row]] = await pool.query(
    'SELECT COUNT(*) AS total FROM profissionais WHERE estabelecimento_id=?', [estabelecimentoId] );
return Number(row?.total || 0);
}

export async function getPlanContext(estabelecimentoId) {
const [rows] = await pool.query(
    "SELECT plan, plan_status, plan_trial_ends_at, plan_active_until, plan_subscription_id, plan_cycle FROM usuarios WHERE id= AND tipo='estabelecimento' LIMIT 1", [estabelecimentoId] );
...
return {
plan, status, cycle, config, trialEndsAt, activeUntil, trial, subscriptionId: row.plan_subscription_id || null, };
}
...
return {
plan, status, billing_cycle: cycle, limits: {
maxMonthlyAppointments: config.maxMonthlyAppointments, maxServices: config.maxServices, maxProfessionals: config.maxProfessionals, maxGalleryImages: config.maxGalleryImages, allowWhatsApp: config.allowWhatsApp, allowAdvancedReports: config.allowAdvancedReports, whatsappIncludedMessages: config.whatsappIncludedMessages ?? null, whatsappMaxMessagesPerAppointment: config.whatsappMaxMessagesPerAppointment || 5, }, features: {
allow_whatsapp: config.allowWhatsApp, allow_advanced: config.allowAdvancedReports, }, trial: {
ends_at: trialEndsAt ? trialEndsAt.toISOString() : null, days_left: trial.daysLeft, warn: trial.warn, active: trial.isTrial, }, active_until: activeUntil ? activeUntil.toISOString() : null, subscription_id: subscriptionId, };
}
