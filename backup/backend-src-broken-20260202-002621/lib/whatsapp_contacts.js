...
const [rows] = await pool.query(
      `SELECT id FROM usuarios WHERE telefone IN (${placeholders}) LIMIT 1`, values );
...
await pool.query(
      `INSERT INTO whatsapp_contacts (recipient_id, cliente_id, last_inbound_at)
       ...
       ON DUPLICATE KEY UPDATE
         ...
         updated_at=CURRENT_TIMESTAMP`, [waId, resolvedClienteId] );
...
const [[row]] = await pool.query(
      'SELECT last_inbound_at FROM whatsapp_contacts WHERE recipient_id= LIMIT 1', [waId] );
