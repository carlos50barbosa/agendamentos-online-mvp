...
try {
await pool.query(
      'UPDATE agendamentos SET estab_reminder_5h_sent_at=NOW(), estab_reminder_5h_msg_id=COALESCE(estab_reminder_5h_msg_id, ?) WHERE id= AND estab_reminder_5h_sent_at IS NULL', [messageId || null, id] );
} catch (e) {
...
if (canWhatsapp) {
const paramModeEnv = String( process.env.WA_TEMPLATE_PARAM_MODE_ESTAB_REMINDER || process.env.WA_TEMPLATE_PARAM_MODE_REMINDER || process.env.WA_TEMPLATE_PARAM_MODE ||
      'quad'
    ).toLowerCase();
const tplName = process.env.WA_TEMPLATE_NAME_ESTAB_REMINDER || process.env.WA_TEMPLATE_NAME_ESTAB || process.env.WA_TEMPLATE_NAME_REMINDER || process.env.WA_TEMPLATE_NAME ||
...
const sendWithParams = async (params) => sendAppointmentWhatsApp({
estabelecimentoId: row?.estabelecimento_id, agendamentoId: row.id, to: telEst, kind: 'estab_reminder_5h', template: { name: tplName, lang: tplLang, bodyParams: params }, });
try {
...
estabelecimentoId: row?.estabelecimento_id, agendamentoId: row.id, to: telEst, kind: 'estab_reminder_5h', message: msgText, });
waMessageId = waMessageId || a2?.provider_message_id || null;
}
      }
} catch (fallbackErr) {
