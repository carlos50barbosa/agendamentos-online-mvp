...
const ACTIVE_APPOINTMENT_STATUSES = ['confirmado', 'pendente', 'pendente_pagamento', 'concluido'];
...
const [rows] = await pool.query(
    "SELECT COUNT(*) AS total FROM agendamentos WHERE estabelecimento_id= AND status IN ('confirmado','pendente','pendente_pagamento','concluido') AND inicio >=  AND inicio < ?", [estabelecimentoId, start, end] );
...
const [anyStatus] = await pool.query(
      'SELECT COUNT(*) AS total FROM agendamentos WHERE estabelecimento_id= AND inicio >=  AND inicio < ?', [estabelecimentoId, start, end] );
...
const [[est]] = await pool.query(
      'SELECT email, telefone, nome, notify_email_estab, notify_whatsapp_estab FROM usuarios WHERE id=?', [estabelecimentoId] );
...
export const __test = {
computeMonthRange, ACTIVE_APPOINTMENT_STATUSES, };
