...
name: tpl.name, lang: tpl.language?.code, };
...
return {
type, text: { length: String(body).length }, };
}
...
function isAllowed(to) {
  ...
  const list = String(process.env.WHATSAPP_ALLOWED_LIST || '') .split(',') .map(x => toDigits(x)) .filter(Boolean);
...
type: payload?.type || 'unknown', to: maskPhone(payload?.to), payload: summarizePayload(payload), }));
...
const logData = (error && typeof error === 'object') || {
          ...
          message: error.message, code: error.code, error_data: error.error_data, fbtrace_id: error.fbtrace_id, } ? : data;
...
type: payload?.type || 'unknown', to: maskPhone(payload?.to), wamid: extractWamid(data), }));
return data;
}
...
throw e;
  ...
  return postGraph({
messaging_product: 'whatsapp', to: dest, type: 'text', text: { body: String(body || 'Olá!') }
});
}
...
throw e;
  ...
  const template = {
name: name || process.env.WA_TEMPLATE_NAME || 'hello_world', language: { code: lang || process.env.WA_TEMPLATE_LANG || 'en_US' }, };
