...
import {
debitWhatsAppMessage, getWhatsAppWalletSnapshot, recordWhatsAppBlocked, WHATSAPP_MAX_MESSAGES_PER_APPOINTMENT, } from './whatsapp_wallet.js';
...
if (!row) return null;
return {
serviceLabel: row.servico_nome || '', dataHoraLabel: brDateTime(row.inicio), estabelecimentoNome: row.estabelecimento_nome || '', };
} catch (err) {
...
template.bodyParams = buildConfirmacaoAgendamentoV2Components({
serviceLabel: ctx?.serviceLabel || rawParams[0], dataHoraLabel: ctx?.dataHoraLabel || rawParams[1], estabelecimentoNome: ctx?.estabelecimentoNome || rawParams[2], });
return template;
}
...
estabelecimentoId, agendamentoId, to, kind = 'appointment', message, template, metadata, }) {
...
estabelecimentoId: estabId, agendamentoId: agId, reason: 'per_appointment_limit', metadata: { kind, sentCount, max: WHATSAPP_MAX_MESSAGES_PER_APPOINTMENT }, });
...
await recordWhatsAppBlocked({
estabelecimentoId: estabId, agendamentoId: agId, reason: 'insufficient_balance', metadata: { kind, wallet }, });
...
let templateToSend = template && template.name || {
      name: template.name, lang: template.lang, bodyParams: template.bodyParams || [], headerImageUrl: template.headerImageUrl, headerDocumentUrl: template.headerDocumentUrl, headerVideoUrl: template.headerVideoUrl, headerText: template.headerText, } : null;
if (templateToSend) {
templateToSend = await ensureConfirmacaoAgendamentoV2BodyParams(templateToSend, {
agendamentoId: agId, estabelecimentoId: estabId, });
}
    resp = await sendWhatsAppSmart({
to, message: message != null ? String(message) : null, template: templateToSend, context: { kind, agendamentoId: agId, estabelecimentoId: estabId }, });
} catch (err) {
const errorCode = err?.code === 'wa_not_connected'  'wa_not_connected' : 'send_failed';
return {
ok: false, sent: false, error: errorCode, detail: err?.message || String(err), wa_status: err?.status, wa_body: err?.body, };
}

  if (resp?.ok === false) {
return {
ok: false, sent: false, error: resp.reason || resp.error || 'send_failed', detail: resp.detail || resp.reason || null, };
...
estabelecimentoId: estabId, agendamentoId: agId, providerMessageId, metadata: { kind, ...metadata }, });
} catch (err) {
