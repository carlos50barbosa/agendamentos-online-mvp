...
const PREFERRED_PUBLIC_PREFIX = normalizePrefix(process.env.ESTABLISHMENT_GALLERY_PUBLIC_PREFIX) || FALLBACK_PUBLIC_PREFIX || DEFAULT_PUBLIC_PREFIX;
...
throw err;
  ...
  await ensureDir();
const filename = `${estabelecimentoId || 'est'}-${Date.now()}-${crypto .randomBytes(5) .toString('hex')}${parsed.ext}`;
