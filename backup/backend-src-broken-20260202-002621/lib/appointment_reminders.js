...
const details = err?.body?.error?.error_data?.details || err?.body?.error?.message || err?.message ||
    '';
...
try {
await pool.query(
      'UPDATE agendamentos SET reminder_8h_sent_at=NOW(), reminder_8h_msg_id=COALESCE(reminder_8h_msg_id, ?) WHERE id= AND reminder_8h_sent_at IS NULL', [messageId || null, id] );
} catch (e) {
...
const paramModeEnv = String( process.env.WA_TEMPLATE_PARAM_MODE_REMINDER || process.env.WA_TEMPLATE_PARAM_MODE ||
    'single'
  ).toLowerCase();
const paramCountHint = Number(process.env.WA_TEMPLATE_REMINDER_PARAMS || process.env.WA_TEMPLATE_PARAMS_REMINDER || NaN);
const tplName = process.env.WA_TEMPLATE_NAME_REMINDER || process.env.WA_TEMPLATE_NAME_CONFIRM || process.env.WA_TEMPLATE_NAME ||
    'confirmacao_agendamento_v2';
const tplLang = process.env.WA_TEMPLATE_LANG || 'pt_BR';
const tplNameLower = String(tplName || '').toLowerCase();
const tplHeaderImage = process.env.WA_TEMPLATE_REMINDER_HEADER_URL || process.env.WA_TEMPLATE_REMINDER_HEADER_IMAGE_URL || process.env.WA_TEMPLATE_HEADER_URL || process.env.WA_TEMPLATE_HEADER_IMAGE_URL ?? null;
...
const quadParams = [ firstName(row?.cliente_nome) || 'cliente', estNomeFriendly, data, hora, ];
...
agendamentoId: row.id, inicio: row.inicio, clienteTelefone: telCli, motivo: reason || 'desconhecido', });
};
const sendTemplateWithParams = async (params) => {
const r = await sendAppointmentWhatsApp({
estabelecimentoId: row.estabelecimento_id, agendamentoId: row.id, to: telCli, kind: 'reminder_8h', template: {
name: tplName, lang: tplLang, bodyParams: params, headerImageUrl: tplHeaderImage || undefined, }, });
