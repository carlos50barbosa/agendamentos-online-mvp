...
import {
parseLocalDate, formatLocalDate, shiftLocalDate, fillDailySeries, normalizeLeadTimeRows, } from '../lib/reporting.js';
const router = Router();
const RANGE_PRESETS = {
  '7d': 7,
  '30d': 30,
...
const VALID_STATUSES = new Set(['confirmado', 'pendente', 'pendente_pagamento', 'cancelado', 'concluido']);
const TZ_OFFSET_MIN = EST_TZ_OFFSET_MIN;
const STATUS_CONFIRMED_SQL = "a.status IN ('confirmado','concluido')";
const STATUS_PLANNED_SQL = "a.status IN ('confirmado','pendente','pendente_pagamento','concluido')";
...
if (!raw) return [];
return String(raw) .split(',') .map((part) => Number(String(part).trim())) .filter((num) => Number.isInteger(num) && num > 0);
}
...
if (set.has('pendente_pagamento')) clauses.push("a.status='pendente_pagamento'");
...
return { error: 'invalid_range', message: 'Data inicial maior que a data final.' };
  ...
  const days = Math.round((Date.UTC(endLocal.year, endLocal.month - 1, endLocal.day) - || Date.UTC(startLocal.year, startLocal.month - 1, startLocal.day)) / (24 * 60 * 60 * 1000)) + 1;
return {
startLocal, endLocal, startUtc: startUtcText, endUtc: endUtcText, startUtcDate: startUtc, endUtcDate: endUtc, days, };
}

function buildBaseFilters({
estId, startUtc, endUtc, statusClause, profissionalId, origem, serviceIds, useServiceExists, }) {
...
params.push(...serviceIds);
  ...
  return {
whereClause: filters.join(' AND '), params, };
}

function buildCsvHeaders() {
return [ { key: 'data', label: 'Data' }, { key: 'cliente', label: 'Cliente' }, { key: 'servico', label: 'Servico' }, { key: 'profissional', label: 'Profissional' }, { key: 'inicio', label: 'Inicio' }, { key: 'fim', label: 'Fim' }, { key: 'status', label: 'Status' }, { key: 'valor', label: 'Valor (BRL)' }, { key: 'origem', label: 'Origem' }, { key: 'criado_em', label: 'Criado em' }, ];
}
