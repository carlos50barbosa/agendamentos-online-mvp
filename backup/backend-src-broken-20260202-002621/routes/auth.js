...
try {
const {
nome, email, senha, tipo, telefone, cep, endereco, numero, complemento, bairro, cidade, estado, data_nascimento, dataNascimento, notifyEmailEstab, notifyWhatsappEstab, } = req.body || {};
...
const trialEndsAt = tipo === 'estabelecimento' && MAX_TRIAL_DAYS > 0 ? ? new Date(now.getTime() + MAX_TRIAL_DAYS * DAY_MS) : null;
...
const [r] = await pool.query(
      ...
      'INSERT INTO usuarios (nome, email, telefone, data_nascimento, cpf_cnpj, cep, endereco, numero, complemento, bairro, cidade, estado, avatar_url, senha_hash, tipo) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)', [
        ...
        nomeTrim, emailTrim, telefoneTrim || null, dataNascimentoValue, cpfCnpjValue, cepDigits || null, enderecoTrim || null, numeroTrim || null, complementoTrim || null, bairroTrim || null, cidadeTrim || null, estadoTrim || null, ?? null, hash, tipo, ]
...
await pool.query(
          ...
          'UPDATE usuarios SET plan=?, plan_status=?, plan_trial_ends_at= WHERE id=?', [planForTrial, 'trialing', trialEndsAt, r.insertId] );
} catch (err) {
...
const user = {
id: r.insertId, nome: nomeTrim, email: emailTrim, telefone: telefoneTrim || null, data_nascimento: dataNascimentoValue, cpf_cnpj: cpfCnpjValue, cep: cepDigits || null, endereco: enderecoTrim || null, numero: numeroTrim || null, complemento: complementoTrim || null, bairro: bairroTrim || null, cidade: cidadeTrim || null, estado: estadoTrim || null, tipo, plan: planForTrial, notify_email_estab: tipo === 'estabelecimento', notify_whatsapp_estab: tipo === 'estabelecimento', plan_status: 'trialing', plan_trial_ends_at: trialEndsAt ? trialEndsAt.toISOString() : null, plan_active_until: null, plan_subscription_id: null, };
...
const htmlAdmin = `
      ...
      `;
const emailJobs = [ { name: 'user', promise: notifyEmail(emailTrim, subjectUser, htmlUser) }, ];
if (adminEmail) {
...
const [rows] = await pool.query(
      ...
      'SELECT id, nome, email, telefone, data_nascimento, cpf_cnpj, cep, endereco, numero, complemento, bairro, cidade, estado, avatar_url, senha_hash, tipo, notify_email_estab, notify_whatsapp_estab, plan, plan_status, plan_trial_ends_at, plan_active_until, plan_subscription_id FROM usuarios WHERE LOWER(email)= LIMIT 1', [email] );
...
const user = {
id: u.id, nome: u.nome, email: u.email, telefone: u.telefone, data_nascimento: u.data_nascimento || null, cpf_cnpj: u.cpf_cnpj || null, cep: u.cep || null, endereco: u.endereco || null, numero: u.numero || null, complemento: u.complemento || null, bairro: u.bairro || null, cidade: u.cidade || null, estado: u.estado || null, avatar_url: u.avatar_url || null, tipo: u.tipo || 'cliente', notify_email_estab: toBool(u.notify_email_estab), notify_whatsapp_estab: toBool(u.notify_whatsapp_estab), plan: u.plan || 'starter', plan_status: u.plan_status || 'trialing', plan_trial_ends_at: u.plan_trial_ends_at ? new Date(u.plan_trial_ends_at).toISOString() : null, plan_active_until: u.plan_active_until ? new Date(u.plan_active_until).toISOString() : null, plan_subscription_id: u.plan_subscription_id || null, };
...
const emailConfirmation = pending ? ? { pending : true, newEmail: pending.new_email, expiresAt: pending.expires_at } ? : null;
...
let {
nome, email, telefone, senhaAtual, senhaNova, cep, endereco, numero, complemento, bairro, cidade, estado, data_nascimento, dataNascimento, notifyEmailEstab, notifyWhatsappEstab, } = req.body || {};
nome = String(nome || '').trim();
email = String(email || '').trim();
telefone = telefone == null ? null : String(telefone).trim();
const cepDigitsRaw = (cep == null ? '' : String(cep)) .replace(/[^0-9]/g, '') .slice(0, 8);
...
await pool.query(
        ...
        'UPDATE usuarios SET nome=?, telefone=?, data_nascimento=?, cpf_cnpj=?, cep=?, endereco=?, numero=?, complemento=?, bairro=?, cidade=?, estado=?, notify_email_estab=?, notify_whatsapp_estab=?, avatar_url= WHERE id=?', [nome, phoneClean || null, dataNascimentoNext, cpfCnpjNext, cepValue, enderecoValue, numeroValue, complementoValue, bairroValue, cidadeValue, estadoValue, nextNotifyEmail ? 1 : 0, nextNotifyWhatsapp ? 1 : 0, nextAvatar, userId] );
...
return res.status(404).json({ error: 'not_found', message: 'Usuario nao encontrado.' });
      ...
      const mergedUser = {
...userRow, nome: userRow.nome || nome, telefone: userRow.telefone || phoneClean, data_nascimento: userRow.data_nascimento || dataNascimentoNext, cpf_cnpj: userRow.cpf_cnpj || cpfCnpjNext, cep: userRow.cep || cepValue, endereco: userRow.endereco || enderecoValue, numero: userRow.numero || numeroValue, complemento: userRow.complemento || complementoValue, bairro: userRow.bairro || bairroValue, cidade: userRow.cidade || cidadeValue, estado: userRow.estado || estadoValue, avatar_url: userRow.avatar_url || nextAvatar || null, notify_email_estab: toBool(userRow.notify_email_estab ?? nextNotifyEmail), notify_whatsapp_estab: toBool(userRow.notify_whatsapp_estab ?? nextNotifyWhatsapp), };
req.user = { ...req.user, ...mergedUser };
return res.json({
ok: true, user: mergedUser, emailConfirmation: { pending: true, newEmail: emailNorm, expiresAt: expiresAt.toISOString() }, });
}
    ...
    await pool.query(
      ...
      'UPDATE usuarios SET nome=?, email=?, telefone=?, data_nascimento=?, cpf_cnpj=?, cep=?, endereco=?, numero=?, complemento=?, bairro=?, cidade=?, estado=?, notify_email_estab=?, notify_whatsapp_estab=?, avatar_url= WHERE id=?', [nome, email, phoneClean || null, dataNascimentoNext, cpfCnpjNext, cepValue, enderecoValue, numeroValue, complementoValue, bairroValue, cidadeValue, estadoValue, nextNotifyEmail ? 1 : 0, nextNotifyWhatsapp ? 1 : 0, nextAvatar, userId] );
const [[userRow]] = await pool.query("SELECT id, nome, email, telefone, data_nascimento, cpf_cnpj, cep, endereco, numero, complemento, bairro, cidade, estado, avatar_url, tipo, notify_email_estab, notify_whatsapp_estab, plan, plan_status, plan_trial_ends_at, plan_active_until, plan_subscription_id FROM usuarios WHERE id= LIMIT 1", [userId]);
if (!userRow) {
return res.status(404).json({ error: 'not_found', message: 'Usuario nao encontrado.' });
}
...
...userRow, notify_email_estab: toBool(userRow.notify_email_estab ?? nextNotifyEmail), notify_whatsapp_estab: toBool(userRow.notify_whatsapp_estab ?? nextNotifyWhatsapp), };
...
return res.status(404).json({ error: 'not_found', message: 'Usuario nao encontrado.' });
    ...
    const normalized = {
...userRow, notify_email_estab: toBool(userRow.notify_email_estab), notify_whatsapp_estab: toBool(userRow.notify_whatsapp_estab), };
...
await pool.query(
          ...
          'INSERT INTO password_resets (user_id, jti, expires_at) VALUES (?,?,?)', [user.id, jti, expires] );
}catch(e){
console.error('[auth/forgot] falha ao registrar token:', e?.message || e);
      ...
      const token = jwt.sign( { sub: user.id, email: user.email, scope: 'pwd_reset' }, secret, { expiresIn: '30m', jwtid: jti }
);
...
const [rows] = await pool.query(
        ...
        'SELECT id, user_id, used_at, expires_at FROM password_resets WHERE jti= LIMIT 1', [String(payload.jti)] );
const rec = rows?.[0];
