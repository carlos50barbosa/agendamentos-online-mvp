...
try {
const sql = `
      SELECT se.id, se.subscription_id, se.event_type, se.gateway_event_id, se.created_at,
             COALESCE(se.payload_json, se.payload) AS payload_json,
...
const id = String( (req.body && (req.body.payment_id || req.body.id)) || (req.query && (req.query.payment_id || req.query.id)) ||
    ''
  ).trim();
