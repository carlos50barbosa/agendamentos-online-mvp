...
import {
getPlanContext, resolvePlanConfig, isDelinquentStatus, } from '../lib/plans.js';
...
const [rows] = await pool.query(
    `SELECT id, ativo FROM profissionais WHERE estabelecimento_id= AND id IN (${placeholders})`, [establishmentId, ...normalized] );
...
const [rows] = await pool.query(
    `SELECT sp.servico_id, p.id, p.nome, p.descricao, p.avatar_url
      ...
      ORDER BY p.nome`, serviceIds );
const grouped = new Map();
rows.forEach((row) => {
if (!grouped.has(row.servico_id)) grouped.set(row.servico_id, []);
grouped.get(row.servico_id).push({
id: row.id, nome: row.nome, descricao: row.descricao, avatar_url: row.avatar_url, });
});
...
const [[service]] = await pool.query(
    'SELECT * FROM servicos WHERE id= AND estabelecimento_id=?', [serviceId, establishmentId] );
...
const estabId = req.query.establishmentId || req.query.estabelecimento_id || req.query.establishment_id ?? null;
...
const [rows] = await pool.query(
      'SELECT * FROM servicos WHERE estabelecimento_id= ORDER BY id DESC', [estId] );
await attachProfessionals(rows);
res.json(rows);
} catch (e) {
...
error: 'invalid_payload', message: 'Preencha nome, duração e selecione pelo menos um profissional para cadastrar o serviço.', });
}
...
error: 'plan_delinquent', message: 'Sua assinatura esta em atraso. Regularize o pagamento para cadastrar novos servicos.', });
}
...
return res.status(400).json({
error: 'missing_professionals', message: 'Selecione pelo menos um profissional para vincular a este serviço.', });
}
...
const [insert] = await conn.query(
      'INSERT INTO servicos (estabelecimento_id, nome, descricao, imagem_url, duracao_min, preco_centavos, ativo) VALUES (?,?,?,?,?,?,?)', [estId, nomeTrim, descricaoTrim || null, imagemUrl, Number.isFinite(duracao) ? Math.max(0, Math.round(duracao)) : 0, Number.isFinite(preco) ? Math.max(0, Math.round(preco)) : 0, isActive ? 1 : 0] );
const serviceId = insert.insertId;
for (const profissionalId of professionalIdsToLink) {
await conn.query(
        'INSERT INTO servico_profissionais (servico_id, profissional_id) VALUES (?,?)', [serviceId, profissionalId] );
}
...
return res.status(400).json({ error: 'invalid_id', message: 'Servico invalido.' });
    ...
    const [[current]] = await pool.query(
      'SELECT * FROM servicos WHERE id= AND estabelecimento_id=?', [serviceId, estId] );
if (!current) return res.status(404).json({ error: 'not_found' });
const updates = {
nome: req.body?.nome != null ? String(req.body.nome).trim() : current.nome, descricao: Object.prototype.hasOwnProperty.call(req.body || {}, 'descricao') ? (req.body.descricao != null ? String(req.body.descricao).trim() : null) ? : current.descricao, duracao_min: req.body?.duracao_min != null ? Number(req.body.duracao_min) : current.duracao_min, preco_centavos: req.body?.preco_centavos != null ? Number(req.body.preco_centavos) : current.preco_centavos, ativo: req.body?.ativo != null ? (toBoolean(req.body.ativo) || 1 : 0) : current.ativo, };
...
return res.status(400).json({
error: 'missing_professionals', message: 'Associe pelo menos um profissional ao serviço.', });
}
...
await conn.beginTransaction();
await conn.query(
      'UPDATE servicos SET nome=?, descricao=?, imagem_url=?, duracao_min=?, preco_centavos=?, ativo= WHERE id= AND estabelecimento_id=?', [updates.nome, updates.descricao, nextImageUrl, updates.duracao_min, updates.preco_centavos, updates.ativo ? 1 : 0, serviceId, estId] );
...
return res.status(400).json({ error: 'invalid_id' });
    ...
    const [[lockedAppointment]] = await pool.query(
      `SELECT a.id
        ...
        LIMIT 1`, [serviceId, estId] );
if (lockedAppointment) {
return res.status(409).json({
error: 'service_has_appointments', message: 'Não é possível excluir um serviço com agendamentos confirmados.', });
}

    const [[row]] = await pool.query(
      'SELECT id, imagem_url FROM servicos WHERE id= AND estabelecimento_id=?', [serviceId, estId] );
const [result] = await pool.query(
      'DELETE FROM servicos WHERE id= AND estabelecimento_id=?', [serviceId, estId] );
