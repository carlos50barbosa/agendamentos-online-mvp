...
const [rows] = await db.query(
    `SELECT * FROM appointment_payments WHERE id= LIMIT 1${lock}`, [id] );
return rows?.[0] || null;
}
...
const lock = forUpdate ? ' FOR UPDATE' : '';
const [rows] = await db.query(
    `SELECT * FROM appointment_payments WHERE provider_payment_id= LIMIT 1${lock}`, [String(providerPaymentId)] );
return rows?.[0] || null;
}

async function markPaymentExpired({ paymentRow, rawPayload = null, connection }) {
await connection.query(
    'UPDATE appointment_payments SET status=?, raw_payload= WHERE id=?', ['expired', rawPayload, paymentRow.id] );
await connection.query(
    "UPDATE agendamentos SET status='cancelado', deposit_expires_at=NOW() WHERE id= AND status='pendente_pagamento'", [paymentRow.agendamento_id] );
}

async function markPaymentPaid({ paymentRow, rawPayload = null, providerReference = null, connection }) {
const payload = rawPayload || null;
await connection.query(
    'UPDATE appointment_payments SET status=?, paid_at=NOW(), raw_payload=?, provider_reference=COALESCE(provider_reference, ?) WHERE id=?', ['paid', payload, providerReference, paymentRow.id] );
const [result] = await connection.query(
    "UPDATE agendamentos SET status='confirmado', deposit_paid_at=NOW() WHERE id= AND status='pendente_pagamento'", [paymentRow.agendamento_id] );
...
paymentRow, rawPayload: safeJson({ event: eventPayload, payment: paymentData }), connection: conn, });
await conn.commit();
return { ok: false, reason: 'expired' };
}

    const affected = await markPaymentPaid({
paymentRow, rawPayload: safeJson({ event: eventPayload, payment: paymentData }), providerReference: paymentData?.external_reference || null, connection: conn, });
await conn.commit();
...
const shouldSync = status === 'pending' && || payment.provider_payment_id && || (!payment.updated_at || new Date(payment.updated_at).getTime() < Date.now() - 15000);
if (shouldSync) {
try {
...
estabelecimento_id: payment.estabelecimento_id, reason: mpAccess.reason || 'missing_token', });
} else {
const mpPayment = await fetchMercadoPagoPayment( payment.provider_payment_id, accessToken ? { accessToken } : undefined );
...
await markPaymentExpired({
paymentRow: locked, rawPayload: safeJson({ payment: mpPayment }), connection: conn, });
}
              await conn.commit();
payment = await fetchPaymentById(id);
} catch (err) {
...
return res.json({
ok: true, id: payment.id, status: normalized, paid: normalized === 'paid', expired: normalized === 'expired' || normalized === 'canceled' || normalized === 'failed', expires_at: payment.expires_at ? new Date(payment.expires_at).toISOString() : null, paid_at: payment.paid_at ? new Date(payment.paid_at).toISOString() : null, amount_centavos: payment.amount_centavos, agendamento_id: payment.agendamento_id, });
} catch (err) {
...
estabelecimento_id: paymentRow.estabelecimento_id, reason: mpAccess.reason || 'missing_token', });
return res.status(200).json({ ok: true, ignored: 'mp_token_missing' });
}

    const payment = await fetchMercadoPagoPayment( resourceId, accessToken ? { accessToken } : undefined );
if (!payment?.id) {
