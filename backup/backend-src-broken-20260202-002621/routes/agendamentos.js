...
const enabled = Boolean(enabledFlag) && || Number.isFinite(percent) && || percent >= 5 && || percent <= 90;
...
String(value) .split(',') .map((part) => part.trim()) .filter(Boolean) .forEach(pushId);
}
...
const rawList = body.servico_ids ? || body.servicos ? || body.service_ids ? || body.services ? || body.serviceIds ? || body.servicoIds ? ?? null;
...
const precoTotal = items.reduce( (sum, item) => sum + || Number( item?.preco_centavos ? || item?.preco_centavos_snapshot ? || item?.preco_snapshot ? || 0

      ), 0

  );
return {
serviceIds: items.map((item) => item.id), serviceNames, serviceLabel: serviceNames.join(' + '), duracaoTotal, precoTotal, };
};
...
const [rows] = await db.query(
    ...
    `SELECT id, nome, duracao_min, preco_centavos
      ...
      WHERE id IN (${placeholders})
        ...
        AND ativo=1`, [...serviceIds, estabelecimentoId] );
...
return {
id: Number(svc.id), nome: svc.nome, duracao_min: Number(svc.duracao_min || 0), preco_centavos: Number(svc.preco_centavos || 0), };
});
...
const [rows] = await db.query(
    ...
    `SELECT servico_id, profissional_id
      ...
      WHERE servico_id IN (${placeholders})`, serviceIds );
const map = new Map();
rows.forEach((row) => {
...
const [rows] = await db.query(
    ...
    `SELECT ai.agendamento_id,
      ...
      ORDER BY ai.agendamento_id, ai.ordem`, appointmentIds );
...
id: Number(row.servico_id), nome: row.servico_nome, ordem: Number(row.ordem) || 0, duracao_min: Number(row.duracao_min || 0), preco_centavos_snapshot: precoCentavosSnapshot, preco_snapshot: precoCentavosSnapshot, });
});
...
id: item.id, nome: item.nome, duracao_min: item.duracao_min, preco_centavos: item.preco_centavos_snapshot || item.preco_snapshot, })));
row.servicos = items;
...
error: 'invalid_payload', message: 'Campos obrigatorios: estabelecimento_id, servico_ids, inicio (ISO).'
...
const depositExpiresAt = depositRequired ? new Date(Date.now() + Number(depositHoldMinutes || DEFAULT_DEPOSIT_HOLD_MINUTES) * 60_000) : null;
...
error: 'mp_not_connected', message: 'Conecte seu Mercado Pago para receber o sinal.', });
}
        console.warn('[agendamentos][deposit] mp fallback', {
estabelecimento_id, reason: mpAccess.reason || 'missing_token', });
}
    }
...
const [[profRow]] = await pool.query(
        ...
        'SELECT id, nome, avatar_url, ativo FROM profissionais WHERE id= AND estabelecimento_id=?', [profissional_id, estabelecimento_id] );
if (!profRow) {
...
db: conn || pool, estabelecimentoId: estabelecimento_id, dateUtc: inicioDate, });
const { startMin, endMin, spansDays } = getLocalRangeMinutes(inicioDate, fimDate);
if (!assertDentroExpediente({
startMin, endMin, abre: expediente.abre, fecha: expediente.fecha, spansDays, breaks: expediente.breaks, })) {
if (conn) {
if (txStarted) {
await conn.rollback();
}
...
estabelecimentoId: estabelecimento_id, planConfig, appointmentDate: inicioDate, });
if (!limitCheck.ok) {
fireAndForget(() => notifyAppointmentLimitReached({
estabelecimentoId: estabelecimento_id, limit: limitCheck.limit, total: limitCheck.total, range: limitCheck.range, planConfig, }));
return res.status(403).json({
error: 'plan_limit_agendamentos', message: formatPlanLimitExceeded(planConfig, 'appointments') || 'Limite de agendamentos atingido para este mês.', details: {
limit: limitCheck.limit, total: limitCheck.total, month: limitCheck.range?.label || null, }, });
}
...
const [r] = await conn.query(
        `INSERT INTO agendamentos
         ...
         VALUES (?,?,?,?,?,?,'pendente_pagamento',?,?,?,?,?)`, [
          req.user.id, estabelecimento_id, primaryServiceId, profissional_id || null, inicioDate, fimDate, totalCentavos, 1, depositPercent, depositCentavos, depositExpiresAt, ]
      );
...
const [payIns] = await conn.query(
        `INSERT INTO appointment_payments
         ...
         VALUES (?,?,?,?,?,?,?)`, [
          appointmentId, estabelecimento_id,
          'deposit',
          'pending', depositCentavos, depositPercent, depositExpiresAt, ]
      );
depositPaymentId = payIns.insertId;
} else {
const [r] = await conn.query(
        'INSERT INTO agendamentos (cliente_id, estabelecimento_id, servico_id, profissional_id, inicio, fim, total_centavos) VALUES (?,?,?,?,?,?,?)', [req.user.id, estabelecimento_id, primaryServiceId, profissional_id || null, inicioDate, fimDate, totalCentavos] );
appointmentId = r.insertId;
}
    const itemValues = serviceItems.map((item, idx) => ([ appointmentId, item.id, idx + 1, Math.max(0, Math.round(item.duracao_min || 0)), Math.max(0, Math.round(item.preco_centavos || 0)), ]));
if (itemValues.length) {
const placeholders = itemValues.map(() => '(?,?,?,?,?)').join(',');
await conn.query(

        `INSERT INTO agendamento_itens (agendamento_id, servico_id, ordem, duracao_min, preco_snapshot) VALUES ${placeholders}`, itemValues.flat() );
}
...
const metadata = {
agendamento_id: String(novo.id), estabelecimento_id: String(estabelecimento_id), type: 'deposit', };
try {
const { payment, pix } = await createMercadoPagoPixPayment({
amountCents: depositCentavos, description: `Sinal - ${serviceLabel}`, externalReference, metadata, notificationUrl: `${apiBase}/payments/webhook`, payerEmail: cli?.email || null, expiresAt: depositExpiresAt, accessToken: mpAccessToken, });
await pool.query(
          'UPDATE appointment_payments SET provider_payment_id=?, provider_reference=?, raw_payload= WHERE id=?', [String(payment.id), externalReference, safeJson(payment), depositPaymentId] );
return res.status(201).json({
id: novo.id, agendamentoId: novo.id, status: novo.status, total_centavos: totalCentavos, deposit_required: 1, deposit_percent: depositPercent, deposit_centavos: depositCentavos, deposit_expires_at: expiresIso, paymentId: depositPaymentId, expiresAt: expiresIso, pix_qr: pix?.qr_code_base64 || null, pix_qr_raw: pix?.qr_code || null, pix_copia_cola: pix?.copia_e_cola || pix?.qr_code || null, pix_ticket_url: pix?.ticket_url || null, amount_centavos: depositCentavos, pix, });
} catch (err) {
console.error('[agendamentos][deposit] erro ao criar PIX:', err?.message || err);
const payload = safeJson({ error: err?.message || String(err) });
await pool.query(
          'UPDATE appointment_payments SET status=?, raw_payload= WHERE id=?', ['failed', payload, depositPaymentId] );
await pool.query(
          "UPDATE agendamentos SET status='cancelado', deposit_expires_at=NOW() WHERE id= AND status='pendente_pagamento'", [novo.id] );
return res.status(502).json({
error: 'payment_create_failed', message: 'Nao foi possivel gerar o PIX do sinal. Tente novamente.', });
}
    }
...
const tplParams = isConfirmV2 || buildConfirmacaoAgendamentoV2Components({
serviceLabel, dataHoraLabel: inicioBR, estabelecimentoNome: estNomeLabel, }) ? : [serviceLabel, inicioBR, estNomeLabel];
...
estabelecimentoId: estabelecimento_id, agendamentoId: novo.id, to: telCli, kind: 'confirm_cli', template: { name: tplName, lang: tplLang, bodyParams: tplParams }, });
} else {
await sendAppointmentWhatsApp({
estabelecimentoId: estabelecimento_id, agendamentoId: novo.id, to: telCli, kind: 'confirm_cli', message: waMsg, template: { name: tplName, lang: tplLang, bodyParams: fallbackBodyParams }, });
}
...
if (/^triple|3$/.test(paramMode)) {
await sendAppointmentWhatsApp({
estabelecimentoId: estabelecimento_id, agendamentoId: novo.id, to: telEst, kind: 'confirm_est', template: { name: tplName, lang: tplLang, bodyParams: tplParams }, });
} else {
await sendAppointmentWhatsApp({
estabelecimentoId: estabelecimento_id, agendamentoId: novo.id, to: telEst, kind: 'confirm_est', message: waMsg, template: { name: tplName, lang: tplLang, bodyParams: fallbackBodyParams }, });
}
    ...
    return res.status(201).json({
id: novo.id, cliente_id: novo.cliente_id, estabelecimento_id: novo.estabelecimento_id, servico_id: novo.servico_id, servico_ids: summary.serviceIds, servico_nome: serviceLabel, servicos: serviceItems.map((item, idx) => {
const precoCentavosSnapshot = Math.max(0, Math.round(item.preco_centavos || 0));
return {
id: item.id, nome: item.nome, duracao_min: item.duracao_min, preco_centavos_snapshot: precoCentavosSnapshot, preco_snapshot: precoCentavosSnapshot, ordem: idx + 1, };
}), buffer_min: APPOINTMENT_BUFFER_MIN, duracao_total: summary.duracaoTotal, duracao_total_com_buffer: duracaoTotalComBuffer, preco_total: summary.precoTotal, total_centavos: totalCentavos, profissional_id: novo.profissional_id, profissional_nome: profissionalRow?.nome || null, profissional_avatar_url: profissionalRow?.avatar_url || null, inicio: novo.inicio, fim: novo.fim, status: novo.status });
} catch (e) {
...
try {
const {
estabelecimento_id: estabelecimentoIdRaw, inicio, nome, email, telefone, profissional_id: profissionalIdRaw, profissionalId, cep, endereco, numero, complemento, bairro, cidade, estado, data_nascimento, dataNascimento, } = req.body || {};
const serviceIds = extractServiceIds(req.body || {});
...
return res.status(400).json({
error: 'invalid_payload', message: 'Campos obrigatorios: servico_ids, inicio, nome, email, telefone.'
    ...
    const cepDigits = (cep == null ? '' : String(cep)) .replace(/[^0-9]/g, '') .slice(0, 8);
...
estabelecimentoId: estabelecimento_id, limit: limitCheck.limit, total: limitCheck.total, range: limitCheck.range, planConfig, }));
return res.status(403).json({
error: 'plan_limit_agendamentos', message: formatPlanLimitExceeded(planConfig, 'appointments') || 'Limite de agendamentos atingido para este mes.', details: {
limit: limitCheck.limit, total: limitCheck.total, month: limitCheck.range?.label || null, }, });
}
...
let userByPhone = null;
{
      ...
      const [urows] = await pool.query(
        ...
        'SELECT id, nome, email, telefone FROM usuarios WHERE LOWER(email)= LIMIT 1', [emailNorm] );
...
const [urows] = await pool.query(
          ...
          'SELECT id, nome, email, telefone FROM usuarios WHERE telefone= LIMIT 1', [candidate] );
if (urows.length) {
userByPhone = urows[0];
break;
}
...
return res.status(409).json({
error: 'cliente_conflito', message: 'Ja existe cliente com este email ou telefone. Revise os dados antes de continuar.', });
}
...
const [r] = await pool.query(
        ...
        "INSERT INTO usuarios (nome, email, telefone, data_nascimento, cep, endereco, numero, complemento, bairro, cidade, estado, senha_hash, tipo) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,'cliente')", [
          ...
          String(nome).slice(0,120), emailNorm, telNorm || null, dataNascimentoValue, cepDigits || null, enderecoTrim || null, numeroTrim || null, complementoTrim || null, bairroTrim || null, cidadeTrim || null, estadoTrim || null, hash, ]
...
return res.status(409).json({ error: 'slot_ocupado' });
    ...
    const [ins] = await conn.query(
      'INSERT INTO agendamentos (cliente_id, estabelecimento_id, servico_id, profissional_id, inicio, fim, total_centavos) VALUES (?,?,?,?,?,?,?)', [userId, estabelecimento_id, primaryServiceId, profissional_id || null, inicioDate, fimDate, totalCentavos] );
const itemValues = serviceItems.map((item, idx) => ([ ins.insertId, item.id, idx + 1, Math.max(0, Math.round(item.duracao_min || 0)), Math.max(0, Math.round(item.preco_centavos || 0)), ]));
if (itemValues.length) {
const placeholders = itemValues.map(() => '(?,?,?,?,?)').join(',');
await conn.query(
...
if (/^triple|3$/.test(paramMode)) {
await sendAppointmentWhatsApp({
estabelecimentoId: req.user.id, agendamentoId: novo.id, to: telCli, kind: 'confirm_cli', template: { name: tplName, lang: tplLang, bodyParams: tplParams }, });
} else {
await sendAppointmentWhatsApp({
estabelecimentoId: req.user.id, agendamentoId: novo.id, to: telCli, kind: 'confirm_cli', message: waMsg, template: { name: tplName, lang: tplLang, bodyParams: fallbackBodyParams }, });
}
...
if (/^triple|3$/.test(paramMode)) {
await sendAppointmentWhatsApp({
estabelecimentoId: req.user.id, agendamentoId: novo.id, to: telEst, kind: 'confirm_est', template: { name: tplName, lang: tplLang, bodyParams: tplParams }, });
} else {
await sendAppointmentWhatsApp({
estabelecimentoId: req.user.id, agendamentoId: novo.id, to: telEst, kind: 'confirm_est', message: waMsg, template: { name: tplName, lang: tplLang, bodyParams: fallbackBodyParams }, });
}
...
return res.status(403).json({
error: 'plan_delinquent', message: 'Sua assinatura esta em atraso. Reagendamentos temporariamente suspensos.', });
}
...
const [[ag]] = await conn.query(
      ...
      `SELECT id, cliente_id, servico_id, profissional_id, status, inicio
        ...
        WHERE id= AND estabelecimento_id=FOR UPDATE`, [id, estId] );
if (!ag) {
if (txStarted && conn) {
await conn.rollback();
}
...
conn = null;
return res.status(409).json({
error: 'reschedule_forbidden_time_limit', message: 'Reagendamento indisponível: horário já iniciado.', });
}
...
return res.status(400).json({ error: 'duracao_invalida', message: 'Duracao do servico invalida.' });
    ...
    const fimDate = new Date(inicioDate.getTime() + duracaoTotalComBuffer * 60_000);
const expediente = await getExpediente({
db: conn || pool, estabelecimentoId: estId, dateUtc: inicioDate, });
const { startMin, endMin, spansDays } = getLocalRangeMinutes(inicioDate, fimDate);
if (!assertDentroExpediente({
startMin, endMin, abre: expediente.abre, fecha: expediente.fecha, spansDays, breaks: expediente.breaks, })) {
if (conn) {
...
await conn.query(
      ...
      'UPDATE agendamentos SET inicio=?, fim= WHERE id= AND estabelecimento_id=?', [inicioDate, fimDate, ag.id, estId] );
const [[updated]] = await conn.query(

      'SELECT inicio, fim FROM agendamentos WHERE id=?', [ag.id] );
const [[cli]] = await conn.query(

      'SELECT nome, email FROM usuarios WHERE id=?', [ag.cliente_id || 0] );
const [[est]] = await conn.query(

      'SELECT nome FROM usuarios WHERE id=?', [estId] );
await conn.commit();
txStarted = false;
conn.release();
conn = null;
const updatedInicioIso = updated?.inicio ? new Date(updated.inicio).toISOString() : inicioDate.toISOString();
...
const [rows] = await pool.query(
      ...
      `UPDATE agendamentos
       ...
       WHERE id= AND cliente_id= AND cliente_confirmou_whatsapp_at IS NULL${limitClause}`, params );
if (!rows.affectedRows) {
      ...
      const [[ag]] = await pool.query(
        ...
        `SELECT cliente_confirmou_whatsapp_at,
          ...
          WHERE id= AND cliente_id=?`, [id, req.user.id] );
if (!ag) {
...
return res.status(409).json({
error: 'cancel_forbidden_after_confirm', message: 'Agendamento já foi confirmado via WhatsApp. Se precisar de ajuda, entre em contato com o estabelecimento.', });
}
...
const paramMode = String( process.env.WA_TEMPLATE_PARAM_MODE_ESTAB_CANCEL || process.env.WA_TEMPLATE_PARAM_MODE_CANCEL || process.env.WA_TEMPLATE_PARAM_MODE ||

          'quad'

        ).toLowerCase();
const tplName = process.env.WA_TEMPLATE_NAME_ESTAB_CANCEL || process.env.WA_TEMPLATE_NAME_CANCEL || process.env.WA_TEMPLATE_NAME ||
...
estabelecimentoId: a?.estabelecimento_id, agendamentoId: id, to: telEst, kind: 'cancel_est', template: { name: tplName, lang: tplLang, bodyParams: tplParams }, });
if (waResult && waResult.ok === false) {
console.warn('[cancel/estab][wa]', waResult?.detail || waResult?.error || 'send_failed');
await sendAppointmentWhatsApp({
estabelecimentoId: a?.estabelecimento_id, agendamentoId: id, to: telEst, kind: 'cancel_est', message: cancelText, });
}
...
const [[ag]] = await pool.query(
      ...
      `SELECT id,
        ...
        WHERE id= AND estabelecimento_id=?`, [id, estId] );
if (!ag) {
return res.status(404).json({ error: 'not_found', message: 'Agendamento não encontrado.' });
}
...
const startTime = ag?.inicio ? new Date(ag.inicio).getTime() : NaN;
const started = (Number.isFinite(secondsToStart) && secondsToStart <= 0) || (Number.isFinite(startTime) && startTime <= Date.now());
if (started) {
return res.status(409).json({
error: 'cancel_forbidden_time_limit', message: 'Cancelamento indisponível: horário já iniciado.', });
}
    ...
    const [rows] = await pool.query(
      ...
      'UPDATE agendamentos SET status="cancelado" WHERE id= AND estabelecimento_id= AND inicio > NOW()', [id, estId] );
if (!rows.affectedRows) {
if (started) {
return res.status(409).json({
error: 'cancel_forbidden_time_limit', message: 'Cancelamento indisponível: horário já iniciado.', });
...
const [[est]] = await pool.query(
      ...
      'SELECT nome, telefone, notify_whatsapp_estab FROM usuarios WHERE id=?', [estId] );
...
const canWhatsappEstCancel = boolPref(est?.notify_whatsapp_estab, true);
    ...
    fireAndForget(async () => {
const paramModeEnv = String( process.env.WA_TEMPLATE_PARAM_MODE_CANCEL || process.env.WA_TEMPLATE_PARAM_MODE ||
...
paramMode = 'quad';
      ...
      const sendParams = async (p) => sendAppointmentWhatsApp({
estabelecimentoId: estId, agendamentoId: id, to: telCli, kind: 'cancel_cli', template: { name: tplName, lang: tplLang, bodyParams: p }, });
...
if (!blockClientWhatsapp && telCli) {
await sendAppointmentWhatsApp({
estabelecimentoId: estId, agendamentoId: id, to: telCli, kind: 'cancel_cli', message: `Seu agendamento ${id} (${serviceLabel  'servico'}) em ${inicioBR} foi cancelado pelo estabelecimento.`, });
}
