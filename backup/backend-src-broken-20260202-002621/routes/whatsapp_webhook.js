...
const raw = msg?.text?.body || msg?.button?.text || msg?.interactive?.button_reply?.title || msg?.interactive?.button_reply?.id || msg?.interactive?.list_reply?.title || msg?.interactive?.list_reply?.id ||
    '';
...
const [ags] = await pool.query(
    `SELECT inicio, fim FROM agendamentos
     WHERE estabelecimento_id= AND status IN ('confirmado','pendente','pendente_pagamento')
       ...
       AND (inicio BETWEEN  AND  OR fim BETWEEN  AND ?)`, [estabId, dayStart, dayEnd, dayStart, dayEnd] );
const [blq] = await pool.query(
    `SELECT inicio, fim FROM bloqueios
     WHERE estabelecimento_id= AND (inicio BETWEEN  AND  OR fim BETWEEN  AND ?)`, [estabId, dayStart, dayEnd, dayStart, dayEnd] );
const slots = [];
...
const [rows] = await pool.query(
      `SELECT a.id, a.inicio, a.reminder_8h_msg_id, a.status, a.estabelecimento_id, u.telefone
       FROM agendamentos a
       ...
       WHERE a.status IN ('confirmado','pendente')
         ...
         AND a.inicio BETWEEN  AND ?`, [lower, upper] );
...
from: fromDigits, ids: candidates.map((c) => c.id), reason, });
}
    const chosen = candidates[0];
console.warn('[wa/confirm-btn][fallback] usando match por telefone (sem reminder_8h_msg_id)', {
from: fromDigits, agendamentoId: chosen?.id, inicio: chosen?.inicio, reason, });
return chosen;
} catch (e) {
...
if (fallback?.id) {
await pool.query(
        'UPDATE agendamentos SET cliente_confirmou_whatsapp_at = COALESCE(cliente_confirmou_whatsapp_at, NOW()) WHERE id= LIMIT 1', [fallback.id] );
...
const [[row]] = await pool.query(
      `SELECT a.id, a.status, a.estabelecimento_id, u.telefone
       FROM agendamentos a
       JOIN usuarios u ON u.id = a.cliente_id
       WHERE a.reminder_8h_msg_id= LIMIT 1`, [contextMessageId] );
if (!row) {
...
return { ok: false, reason: 'not_confirmable', status: statusNorm };
    ...
    await pool.query(
      'UPDATE agendamentos SET cliente_confirmou_whatsapp_at = COALESCE(cliente_confirmou_whatsapp_at, NOW()) WHERE id= LIMIT 1', [row.id] );
...
const error = Array.isArray(status?.errors) && status.errors.length || {
              code: status.errors[0]?.code, title: status.errors[0]?.title, details: status.errors[0]?.details, } ? : null;
console.log('[wa/webhook/status]', JSON.stringify({
wamid: status?.id || null, status: status?.status || null, recipient_id: status?.recipient_id || null, error, }));
});
...
recordWhatsAppInbound({ recipientId: from }).catch((err) => console.warn('[wa/webhook][inbound] failed to record', err?.message || err) );
const inboundText = getInboundText(msg);
const confirmText = normalizeConfirmText(inboundText);
console.log('[wa][inbound]', { type: msg?.type || null, extracted: inboundText?.slice(0, 30) });
const isConfirm = confirmText === 'confirmar' || confirmText.startsWith('confirmar ');
...
if (isConfirm || buttonPayload || contextMsgId) {
try {
const recorded = await tryRecordReminderConfirmation({ contextMessageId: contextMsgId, fromDigits: from });
if (recorded?.ok) {
console.log('[wa][confirm]', {
agendamentoId: recorded?.agendamentoId || null, estabelecimentoId: recorded?.estabelecimentoId || null, via: msg?.type || null, });
...
console.warn('[wa][confirm] failed', err?.message || err);
return res.sendStatus(200);
}
    }
