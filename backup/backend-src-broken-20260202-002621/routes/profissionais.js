...
import {
getPlanContext, isDelinquentStatus, } from '../lib/plans.js';
...
router.post(
  '/', auth, isEstabelecimento, ensureWithinProfessionalLimit({
isActivating: (req) => toBoolean(req.body?.ativo || true) === true, }), async (req, res) => {
try {
const estId = req.user.id;
...
error: 'plan_delinquent', message: 'Sua assinatura esta em atraso. Regularize o pagamento para cadastrar profissionais.', });
}
...
const [insert] = await pool.query(
      'INSERT INTO profissionais (estabelecimento_id, nome, descricao, avatar_url, ativo) VALUES (?,?,?,?,?)', [estId, nome, descricao || null, avatarUrl, isActive ? 1 : 0] );
const [[row]] = await pool.query(
      'SELECT id, estabelecimento_id, nome, descricao, avatar_url, ativo, created_at FROM profissionais WHERE id=?', [insert.insertId] );
return res.json(row);
} catch (err) {
...
const { id } = req.params;
const [[row]] = await pool.query(
      'SELECT id, nome, descricao, avatar_url, ativo FROM profissionais WHERE id= AND estabelecimento_id=?', [id, estId] );
...
router.put(
  '/:id', auth, isEstabelecimento, loadProfessional, ensureWithinProfessionalLimit({
isActivating: (req) => {
...
await pool.query(
      'UPDATE profissionais SET nome=?, descricao=?, avatar_url=?, ativo= WHERE id= AND estabelecimento_id=?', [nextNome, nextDescricao || null, nextAvatar, nextAtivo, id, estId] );
const [[updated]] = await pool.query(
      'SELECT id, estabelecimento_id, nome, descricao, avatar_url, ativo, created_at FROM profissionais WHERE id=?', [id] );
res.json(updated);
} catch (err) {
...
const { id } = req.params;
const [[row]] = await pool.query(
      'SELECT id, avatar_url FROM profissionais WHERE id= AND estabelecimento_id=?', [id, estId] );
if (!row) return res.status(404).json({ error: 'not_found' });
