...
if ( !Number.isFinite(check.getTime()) || check.getUTCFullYear() !== year || check.getUTCMonth() + 1 !== month || check.getUTCDate() !== day ) {
return null;
}
...
const rawList = query.servico_ids ? || query.servicos ? || query.service_ids ? || query.services ? || query.serviceIds ? || query.servicoIds ? ?? null;
...
const durationRaw = req.query.duracao_total ? || req.query.duracaoTotal ? || req.query.duration_min ? || req.query.duration ? ?? null;
...
const [rows] = await pool.query(
        ...
        `SELECT id, duracao_min
          ...
          WHERE id IN (${placeholders})
            ...
            AND ativo=1`, [...serviceIds, establishmentId] );
...
durationMinutes = INTERVAL_MIN;
    ...
    const rangeStartUtc = makeUtcFromLocalYMDHM( weekStartLocal.year, weekStartLocal.month, weekStartLocal.day, 0, 0, EST_TZ_OFFSET_MIN );
...
const [profileRows] = await pool.query(
      ...
      `SELECT horarios_json FROM estabelecimento_perfis WHERE estabelecimento_id =  LIMIT 1`, [establishmentId] );
const horariosJson = profileRows?.[0]?.horarios_json || null;
const workingRules = buildWorkingRules(horariosJson);
const toIntervals = (rows) => (rows || []) .map((row) => [new Date(row.inicio).getTime(), new Date(row.fim).getTime()]) .filter(([startMs, endMs]) => Number.isFinite(startMs) && Number.isFinite(endMs));
...
const dayStartUtc = makeUtcFromLocalYMDHM( localDay.year, localDay.month, localDay.day, 0, 0, EST_TZ_OFFSET_MIN );
...
const intervals = [];
if ( !prevExpediente.closed && || Number.isFinite(prevExpediente.startMinutes) && || Number.isFinite(prevExpediente.endMinutes) && || prevExpediente.startMinutes > prevExpediente.endMinutes && || prevExpediente.endMinutes > 0 ) {
const prevBreaks = Array.isArray(prevExpediente.breaks) ? prevExpediente.breaks : [];
const earlyBreaks = prevBreaks.filter( ([startMin, endMin]) => Number.isFinite(startMin) && || Number.isFinite(endMin) && || startMin < prevExpediente.startMinutes );
intervals.push({
start: 0, end: prevExpediente.endMinutes, closeLimit: prevExpediente.endMinutes, breaks: earlyBreaks, });
}
...
intervals.push({
start: expediente.startMinutes, end: expediente.endMinutes, closeLimit: expediente.endMinutes, breaks: Array.isArray(expediente.breaks) ? expediente.breaks : [], });
...
intervals.push({
start: expediente.startMinutes, end: DAY_MINUTES, closeLimit: DAY_MINUTES + expediente.endMinutes, breaks: lateBreaks, });
}
      }
...
const slotStartUtc = makeUtcFromLocalYMDHM( localDay.year, localDay.month, localDay.day, hour, minuteOfHour, EST_TZ_OFFSET_MIN );
...
const bloqueadoHorario = Array.isArray(interval.breaks) && || interval.breaks.some(([startMin, endMin]) => minute < endMin && slotEndWindow > startMin);
...
error: 'plan_delinquent', message: 'Sua assinatura esta em atraso. Ajustes de agenda estao temporariamente bloqueados.'
...
const e = new Date(s.getTime() + INTERVAL_MIN * 60000);
    ...
    // Ja existe bloqueio exato para esse intervaloconst [rows] = await pool.query(
...
return res.json({ ok: true, action: 'liberado' });
      ...
      await pool.query(
        ...
        `INSERT INTO bloqueios (estabelecimento_id, inicio, fim) VALUES (?,?,?)`, [req.user.id, s, e] );
