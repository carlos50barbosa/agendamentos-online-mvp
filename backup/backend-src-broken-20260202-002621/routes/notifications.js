...
return {
allowed: false, plan, status, message: 'Sua assinatura está em atraso. Regularize o pagamento para continuar enviando notificacões.', };
}

  if (config && config.allowWhatsApp === false) {
return {
allowed: false, plan, status, message: 'Seu plano atual não inclui notificacões por WhatsApp.', };
}

  return {
allowed: true, plan, status, };
}
