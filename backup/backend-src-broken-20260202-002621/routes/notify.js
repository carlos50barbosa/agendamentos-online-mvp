...
const fallbackName = templateNameFallback || process.env.WA_TEMPLATE_NAME_FALLBACK || process.env.WA_TEMPLATE_NAME ||
      'hello_world';
const payloadText = message || text || 'Teste via Cloud API';
const { result, meta } = await sendWhatsAppSmart({
to, text: payloadText, templateNameFallback: fallbackName, returnMeta: true, });
res.json({
ok: true, decision: meta?.decision || null, window_open: meta?.window_open ?? null, force_template: meta?.force_template || false, wamid: meta?.wamid || null, data: result, });
} catch (e) {
