...
await pool.query(
      ...
      'INSERT INTO otp_codes (request_id, channel, value, code_hash, expires_at, ip_addr) VALUES (?,?,?,?,?,?)', [requestId, channel, value, hash, expires, ip] );
...
if (!ok) {
try { await pool.query('UPDATE otp_codes SET attempts=attempts+1 WHERE id=?', [rec.id]); } catch {} return res.status(400).json({ error : 'invalid_code' });
}
