...
const paths = withApiPrefix || ['/api/webhook/mercadopago', '/webhook/mercadopago'] // aceita ambos
...
try {
      ...
      const q = req.query || {};
const b = req.body || {};
const resourceId = q['data.id'] || b?.data?.id || q.id || b.id || b.resource ?? null;

      // console.log('[MP] webhook hit', { path: paths, query: q, headers: req.headers });
      console.log('[MP] webhook hit', {
url: req.originalUrl, x_request_id: req.headers['x-request-id'], x_signature: req.headers['x-signature'], query: q, });
if (!resourceId) {
