...
const precoTotal = items.reduce( (sum, item) => sum + Number(item?.preco_centavos || item?.preco_snapshot || 0), 0
...
id: Number(row.servico_id), nome: row.servico_nome, ordem: Number(row.ordem) || 0, duracao_min: Number(row.duracao_min || 0), preco_snapshot: Number(row.preco_snapshot || 0), });
});
...
const [[ag]] = await pool.query(
      ...
      `SELECT a.id, a.inicio, a.estabelecimento_id, a.servico_id, a.profissional_id,
        ...
        WHERE a.id= LIMIT 1`, [appointmentId] );
if (!ag) return;
...
const [[est]] = await pool.query(
      ...
      'SELECT email, telefone, nome, notify_email_estab, notify_whatsapp_estab FROM usuarios WHERE id=?', [ag.estabelecimento_id] );
const [tmplRows] = await pool.query(

      'SELECT email_subject, email_html, wa_template FROM estab_messages WHERE estabelecimento_id=?', [ag.estabelecimento_id] );
let profissionalRow = null;
if (ag.profissional_id != null) {
const [[profRow]] = await pool.query(

        'SELECT nome FROM profissionais WHERE id= AND estabelecimento_id=?', [ag.profissional_id, ag.estabelecimento_id] );
...
await sendAppointmentWhatsApp({
estabelecimentoId: ag.estabelecimento_id, agendamentoId: ag.id, to: telEst, kind: 'confirm_est', message: waMsgEst, template: { name: tplName, lang: tplLang, bodyParams: fallbackBodyParams }, });
}
...
try {
const {
estabelecimento_id, inicio, nome, email, telefone, otp_token, profissional_id: profissionalIdRaw, profissionalId, cep, endereco, numero, complemento, bairro, cidade, estado, data_nascimento, dataNascimento, } = req.body || {};
...
error: 'invalid_payload', message: 'Campos obrigatorios: estabelecimento_id, servico_ids, inicio, nome, email, telefone.'
...
db: pool, estabelecimentoId: estabelecimento_id, dateUtc: inicioDate, });
...
error: 'plan_limit_agendamentos', message: formatPlanLimitExceeded(planConfig, 'appointments') || 'Limite de agendamentos atingido para este mes.', details: { limit: limitCheck.limit, total: limitCheck.total, month: limitCheck.range?.label || null }, });
}
...
const ok = (payload.ch === 'email' && String(payload.v || '').toLowerCase() === emailNorm) || (payload.ch === 'phone' && String(payload.v || '') === telDigits);
...
return res.status(409).json({
error: 'cliente_conflito', message: 'Já existe cliente com este email ou telefone. Revise os dados antes de continuar.', });
}
...
return res.status(409).json({ error: 'slot_ocupado' });
    ...
    const origem = normalizeOrigem( req.body?.origem ? || req.body?.canal ? || req.body?.channel ? || req.body?.utm_source ? || req.body?.utmSource ? || req.body?.source ? || req.query?.origem ? || req.query?.canal ? || req.query?.utm_source ? || req.query?.utmSource ? || req.query?.source ) || 'site';
...
const [ins] = await conn.query(
        `INSERT INTO agendamentos
         ...
         VALUES (?,?,?,?,?,?,'pendente_pagamento', ?, ?, 1, ?, ?, ?)`, [
          userId, estabelecimento_id, primaryServiceId, profissional_id || null, inicioDate, fimDate, origem, totalCentavos, depositPercent, depositCentavos, depositExpiresAt, ]
      );
...
const [ins] = await conn.query(
        `INSERT INTO agendamentos
         ...
         VALUES (?,?,?,?,?,?,'confirmado', ?, NULL, NULL, NOW(), ?)`, [userId, estabelecimento_id, primaryServiceId, profissional_id || null, inicioDate, fimDate, origem, totalCentavos] );
appointmentId = ins.insertId;
}
...
const metadata = {
agendamento_id: String(appointmentId), estabelecimento_id: String(estabelecimento_id), type: 'deposit', };
try {
const { payment, pix } = await createMercadoPagoPixPayment({
amountCents: depositCentavos, description: `Sinal - ${serviceLabel}`, externalReference, metadata, notificationUrl: `${apiBase}/payments/webhook`, payerEmail: emailNorm || null, expiresAt: depositExpiresAt, accessToken: mpAccessToken, });
await pool.query(
...
await pool.query(
          "UPDATE agendamentos SET status='cancelado', deposit_expires_at=NOW() WHERE id= AND status='pendente_pagamento'", [appointmentId] );
...
return res.status(201).json({
id: appointmentId, cliente_id: userId, estabelecimento_id, servico_id: primaryServiceId, servico_ids: summary.serviceIds, servico_nome: summary.serviceLabel || serviceLabel, servicos: serviceItems, duracao_total: summary.duracaoTotal, preco_total: summary.precoTotal, total_centavos: totalCentavos, profissional_id: profissional_id || null, inicio: inicioDate, fim: fimDate, status: 'confirmado', confirm_expires_at: null, });
} catch (e) {
...
title: 'Token invalido', message: 'Link de confirmacao invalido ou ausente.', }));
}

    const tokenHash = hashToken(token);
const [[ag]] = await pool.query(

      'SELECT id, status, public_confirm_expires_at FROM agendamentos WHERE public_confirm_token_hash= LIMIT 1', [tokenHash] );
if (!ag) {
return res.status(404).send(renderConfirmPage({
title: 'Agendamento nao encontrado', message: 'Este link nao e valido ou ja expirou.', }));
}

    if (ag.status === 'confirmado') {
return res.status(200).send(renderConfirmPage({
title: 'Agendamento confirmado', message: 'Seu agendamento ja estava confirmado.', }));
}

    if (ag.status === 'cancelado') {
return res.status(410).send(renderConfirmPage({
title: 'Confirmacao expirada', message: 'Este agendamento foi cancelado por falta de confirmacao.', }));
}

    if (ag.public_confirm_expires_at && new Date(ag.public_confirm_expires_at).getTime() < Date.now()) {
await pool.query(

        "UPDATE agendamentos SET status='cancelado' WHERE id= AND status='pendente'", [ag.id] );
return res.status(410).send(renderConfirmPage({
title: 'Confirmacao expirada', message: 'Este agendamento foi cancelado por falta de confirmacao.', }));
}
    ...
    const [r] = await pool.query(
      ...
      "UPDATE agendamentos SET status='confirmado', public_confirmed_at=NOW(), public_confirm_expires_at=NULL WHERE id= AND status='pendente'", [ag.id] );
if (r?.affectedRows) {
notifyPublicConfirmedAppointment(ag.id);
}

    return res.status(200).send(renderConfirmPage({
title: 'Agendamento confirmado', message: 'Confirmacao registrada com sucesso.', }));
} catch (e) {
console.error('[public/agendamentos][confirm]', e?.message || e);
return res.status(500).send(renderConfirmPage({
title: 'Erro na confirmacao', message: 'Nao foi possivel confirmar agora. Tente novamente.', }));
}
