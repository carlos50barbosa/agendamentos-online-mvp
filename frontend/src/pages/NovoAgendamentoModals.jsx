...
export default function NovoAgendamentoModals(props) {
  ...
  const depositStatus = String(depositModal?.status || "").toLowerCase();
  const depositPaid = depositStatus === "paid";
  const depositExpired = depositStatus === "expired";
  const depositPending = depositModal?.open && !depositPaid && !depositExpired;
  const depositStatusTone = depositPaid ? "success" : depositExpired ? "error" : "pending";
  const depositStatusText = depositPaid
     ? "Pagamento confirmado"
    : depositExpired
     ? "Tempo esgotado, agendamento cancelado"
    : "Aguardando pagamento do sinal";
  const depositStatusIcon = depositPaid ? "OK" : depositExpired ? "!" : "...";
      ...
      {infoModalOpen && selectedEstablishment && (
            ...
            {depositModal?.open && (
              <Modal
                ...
                closeButton
                actions={[
                  depositPending && depositTicketUrl ? (
                    <a
                      key="open"
                      className="btn btn--primary"
              ...
              >
                <div className="pix-checkout">
                    ...
                    <div className="pix-checkout__code">
                      ...
                      <div className="pix-checkout__code-actions">
                        <button
                          ...
                          onClick={() => handleCopyPixCode(depositPixCode)}
                        >
                          Copiar chave
                        </button>
                      </div>
                  ...
                  {depositPending && depositCountdown && (
                    <p className="muted pix-checkout__expires">Tempo restante: {depositCountdown}</p>
                  )}
