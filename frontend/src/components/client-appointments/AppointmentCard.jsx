...
export default function AppointmentCard({
  ...
  return (
    <article
      className={`tw-rounded-2xl tw-border tw-border-[color:var(--brand-border,#E7E5F5)] tw-bg-white tw-p-4 tw-shadow-sm ${className}`.trim()}
    >
      <header className="tw-flex tw-flex-wrap tw-items-start tw-justify-between tw-gap-2">
        <div className="tw-min-w-0 tw-flex-1">
          <h3 className="tw-m-0 tw-text-sm tw-font-semibold tw-text-ink tw-break-words">{appointment.serviceLabel}</h3>
          <p className="tw-m-0 tw-mt-1 tw-text-xs tw-break-words" style={{ color: 'var(--muted-ink, #6B7280)' }}>{appointment.establishmentLabel}</p>
        </div>
        <StatusBadge status={appointment.effectiveStatus} className="tw-max-w-full tw-shrink-0" />
      </header>

      <p className="tw-m-0 tw-mt-3 tw-flex tw-items-center tw-gap-2 tw-text-sm tw-font-medium" style={{ color: 'var(--ink, #1E1B4B)' }}>
      ...
      <div className="tw-mt-4 tw-grid tw-grid-cols-1 tw-gap-2 sm:tw-grid-cols-2">
        <Button
          ...
          className="tw-w-full tw-min-w-0 tw-whitespace-normal tw-rounded-xl tw-px-3 tw-py-1.5"
