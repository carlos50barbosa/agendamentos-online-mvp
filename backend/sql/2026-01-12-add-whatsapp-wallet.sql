...
-- 5) Garantir indices/uniques da tabela whatsapp_wallet_transactions em bases antigas
SET @sql := IF(
  EXISTS (
    SELECT 1 FROM INFORMATION_SCHEMA.STATISTICS
    WHERE TABLE_SCHEMA = @db AND TABLE_NAME = 'whatsapp_wallet_transactions' AND INDEX_NAME = 'uk_wa_tx_provider_msg'
  ),
  'SELECT 0',
  'ALTER TABLE whatsapp_wallet_transactions ADD UNIQUE INDEX uk_wa_tx_provider_msg (provider_message_id)'
);
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
...
SET @sql := IF(
  EXISTS (
    SELECT 1 FROM INFORMATION_SCHEMA.STATISTICS
    WHERE TABLE_SCHEMA = @db AND TABLE_NAME = 'whatsapp_wallet_transactions' AND INDEX_NAME = 'uk_wa_tx_kind_payment'
  ),
  'SELECT 0',
  'ALTER TABLE whatsapp_wallet_transactions ADD UNIQUE INDEX uk_wa_tx_kind_payment (kind, payment_id)'
);
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
...
SET @sql := IF(
  EXISTS (
    SELECT 1 FROM INFORMATION_SCHEMA.STATISTICS
    WHERE TABLE_SCHEMA = @db AND TABLE_NAME = 'whatsapp_wallet_transactions' AND INDEX_NAME = 'uk_wa_tx_kind_cycle'
  ),
  'SELECT 0',
  'ALTER TABLE whatsapp_wallet_transactions ADD UNIQUE INDEX uk_wa_tx_kind_cycle (kind, estabelecimento_id, cycle_start)'
);
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
