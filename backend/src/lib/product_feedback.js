 ...
 */
export function isNpsEligible({ contaCriadaEm, agendamentos = 0, ultimaRespostaEm = null, agora = new Date() } = {}) {
  ...
  if (ultimaRespostaEm) {
    const last = ultimaRespostaEm instanceof Date ? ultimaRespostaEm : new Date(ultimaRespostaEm);
    if (Number.isFinite(last.getTime())) {
      const dias = (now.getTime() - last.getTime()) / 86400000;
      if (dias < NPS_COOLDOWN_DIAS) return { eligible: false, reason: 'respondeu_recentemente' };
    }
  }
