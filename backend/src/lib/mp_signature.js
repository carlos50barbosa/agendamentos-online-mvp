...
function resolveWebhookId(req) {
  ...
  const body = req?.body || {}
  const candidates = [
    { value: query?.id, source: 'query.id' },
    { value: query?.['data.id'], source: 'query.data.id' },
    { value: body?.data?.id, source: 'body.data.id' },
    { value: body?.id, source: 'body.id' },
  ]
  for (const candidate of candidates) {
    const normalized = String(candidate.value ?? '').trim()
    if (normalized) return { id: normalized, source: candidate.source }
  }
...
function getHeaderPrefix(value) {
  ...
  return trimmed.slice(0, HEADER_PREFIX_LENGTH)
}

function getMissingReason(field) {
  switch (field) {
    case 'x-signature':
      return 'missing_x_signature'
    case 'ts':
      return 'missing_ts'
    case 'v1':
      return 'missing_v1'
    case 'x-request-id':
      return 'missing_x_request_id'
    case 'id':
      return 'missing_id'
    default:
...
function buildBaseLog({
  ...
  const tsStr = String(tsValue || '').trim()
  return {
    url: url || null,
    topic: topic || null,
    x_request_id: requestId || null,
    x_request_id_present: Boolean(requestId),
    x_signature_present: Boolean(signaturePresent),
    x_signature_prefix: signaturePrefix || null,
    id: id || null,
    id_source: idSource || null,
    ts: tsStr || null,
    ts_len: tsStr ? tsStr.length : null,
    ts_is_10_digits: /^\d{10}$/.test(tsStr),
    ts_is_13_digits: /^\d{13}$/.test(tsStr),
  }
}
...
export function verifyMercadoPagoWebhookSignature(req) {
  ...
  const signaturePresent = isHeaderPresent(signatureHeader)
  const signaturePrefix = getHeaderPrefix(signatureHeader)
  const { ts, v1 } = parseMercadoPagoSignatureHeader(signatureHeader)
  const tsValue = String(ts || '').trim()
  const v1Value = String(v1 || '').trim()
  const requestId = String(headers['x-request-id'] || '').trim()
  const { id, source: idSource } = resolveWebhookId(req)
  ...
  const missingFields = []
  if (!signaturePresent) missingFields.push('x-signature')
  if (!tsValue) missingFields.push('ts')
  if (!v1Value) missingFields.push('v1')
  if (!requestId) missingFields.push('x-request-id')
  if (!id) missingFields.push('id')
  if (missingFields.length) {
    const reason = getMissingReason(missingFields[0])
  ...
  const secrets = [
    { label: 'SECRET_1', value: normalizeSecret(process.env.MERCADOPAGO_WEBHOOK_SECRET) },
    { label: 'SECRET_2', value: normalizeSecret(process.env.MERCADOPAGO_WEBHOOK_SECRET_2) },
  ...
  const v1Prefix = v1Value ? v1Value.slice(0, SIGNATURE_PREFIX_LENGTH) : null
  for (const secret of secrets) {
    const expected = createHmac('sha256', secret.value).update(manifest).digest('hex')
    const expectedPrefix = expected.slice(0, SIGNATURE_PREFIX_LENGTH)
    expectedPrefixes.push({ label: secret.label, expected_prefix: expectedPrefix })
    if (safeTimingCompareHex(expected, v1Value)) {
      const usedSecretLast4 = secret.value.slice(-4) || null
      console.info('[billing:webhook] signature_ok', {
        ...baseLog,
        manifest,
        used_secret_last4: usedSecretLast4,
  ...
  console.warn('[billing:webhook] signature_mismatch', {
    ...
    hint:
      'provavel secret errado no painel/endpoint ou ambiente (sandbox vs prod) / credencial diferente para subscription',
  })
  return {
    ok: false,
